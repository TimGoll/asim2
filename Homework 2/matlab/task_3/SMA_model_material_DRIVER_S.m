...
%%%%%% CHANGED HERE %%%%%%
f_stress = @(t) 0.5*10^9;  %   Stress function over time t [Pa]
f_temperature = @(t) 293 + 30 * ( sin(2*pi*t/10) + 1 );    %   Temperature function over time t
%%%%%%%%%%%%%%%%%%%%%%%%%%
...
[t,y] = ode15s('SMA_model_material_S',tspan,y_initial,options);
...
for i = 1:length(t)
    stress(i) = f_stress(t(i));
    temp(i) = f_temperature(t(i));
end

if length(stress) == 1
    stress = stress*ones(size(t));
end
if length(temp) == 1
    temp = temp*ones(size(t));
end
...
strain = stress.*( (xpls + xmns)/E_M  + (xaus)/E_A )+ xpls.*epsilon_T - xmns.*epsilon_T;
